...
* The `onBeforeUnload` event could be used to evade frame busting code. This event is called when the frame busting code wants to destroy the iframe by loading the URL in the whole web page and not only in the iframe. The handler function returns a string that is prompted to the user asking to confirm if they want to leave the page. When this string is displayed to the user it is likely to cancel the navigation, defeating target’s frame busting attempt.
