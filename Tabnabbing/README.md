...
When tabnabbing, the attacker searches for links that are inserted into the website and are under their control. Such links may be contained in a forum post, for example. Once they have found this kind of functionality, it checks that the link's `rel` attribute does not contain the value `noopener` and the target attribute contains the value `_blank`. If this is the case, the website is vulnerable to tabnabbing.

## How to exploit 
```
1. Attacker posts a link to a website under their control that contains the following JS code: window.opener.location = "http://evil.com"
2. They trick the victim into visiting the link, which is opened in the browser in a new tab.
...
4. If the victim opens the background tab again and doesn't look at the address bar, it may happen that they think they are logged out, because a login page appears, for example.
